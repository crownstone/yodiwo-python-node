class Port(object):
    ...
    def fromCloud(cls, dct):
        if all(field in dct for field in
               ("PortKey", "Name", "Description", "ioDirection", "Type", "State", "RevNum", "ConfFlags")):
            port = cls(dct["PortKey"], dct["Name"], dct["Description"], dct["ioDirection"], dct["Type"], dct["State"],
                       dct["RevNum"], dct["ConfFlags"])
            return port
        else:
            return None
